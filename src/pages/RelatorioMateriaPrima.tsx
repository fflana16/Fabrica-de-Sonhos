...
import { useSistemas, MateriaPrima } from '../SistemasContext';
import { toast } from 'sonner';
import { 
  Search, Plus, Eye, Trash2, Edit, X, Filter,
  ArrowUpDown, TrendingDown, TrendingUp, Package, Ruler, DollarSign
} from 'lucide-react';
  ...
  <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
    <div className="bg-white/95 backdrop-blur-xl w-full max-w-3xl rounded-[2.5rem] p-10 shadow-2xl border border-gold/40 relative animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
      ...
      <div className="flex items-center gap-6 mb-10 border-b border-gold/30 pb-8">
        ...
        <div>
          <h3 className="text-3xl font-serif font-bold text-gold-dark mb-1">{mp.nome}</h3>
          <p className="text-sm text-slate-500 font-semibold tracking-widest uppercase">
            Código: <span className="text-gold-dark">{mp.codigo}</span> • Cadastrado em {mp.dataCadastro}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-6">
          <h4 className="text-xs font-black text-gold-dark uppercase tracking-[0.2em] border-l-4 border-gold pl-3">Dimensões</h4>
          <div className="space-y-4">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-gold-dark uppercase tracking-wider">Largura x Altura</span>
              <p className="text-base text-slate-800 font-medium">{mp.largura} x {mp.altura} mm</p>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-gold-dark uppercase tracking-wider">Área Total</span>
              <p className="text-base text-slate-800 font-medium">{mp.areaTotal.toLocaleString('pt-BR')} mm²</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-gold-dark uppercase tracking-wider">Custo da Chapa</span>
              <p className="text-base text-slate-800 font-medium">R$ {mp.custoChapa.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-gold-dark uppercase tracking-wider">Custo por mm²</span>
              <p className="text-base text-gold-dark font-mono font-bold">R$ {mp.custoPorMm2.toFixed(6).replace('.', ',')}</p>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-gold-dark uppercase tracking-wider">Início Validade Custo</span>
              <p className="text-sm text-slate-800 font-medium">
                {mp.historicoCustos?.[mp.historicoCustos.length - 1]?.data || mp.dataCadastro}
              </p>
            </div>
            <div className="flex flex-col gap-1">
              <span className="text-[10px] font-bold text-gold-dark uppercase tracking-wider">Operador Responsável</span>
              <p className="text-sm text-slate-800 font-medium">
                {mp.historicoCustos?.[mp.historicoCustos.length - 1]?.operador || 'Sistema'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="max-h-48 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {(mp.historicoCustos || []).slice().reverse().map((h, idx) => (
...
export const RelatorioMateriaPrima = ({ 
  ...
  const [mpParaExcluir, setMpParaExcluir] = useState<MateriaPrima | null>(null);

  const filteredAndSortedMP = useMemo(() => {
    let result = materiasPrimas.filter(mp => {
      const matchesStatus = statusFilter === 'TODOS' || mp.status === statusFilter;
      const matchesSearch = 
    ...
    if (sortOrder === 'desc') {
      result.sort((a, b) => b.custoPorMm2 - a.custoPorMm2);
    } else if (sortOrder === 'asc') {
      result.sort((a, b) => a.custoPorMm2 - b.custoPorMm2);
    }

    return result;
  }, [materiasPrimas, searchTerm, sortOrder, statusFilter]);
  ...
  const handleEdit = (mp: MateriaPrima) => {
    if (mp.status === 'ELIMINADO') return;
    setMateriaPrimaParaEditar(mp);
    onNavigate('CadastroMateriaPrima');
  };

  const confirmDelete = () => {
    if (!mpParaExcluir) return;
    removerMateriaPrima(mpParaExcluir.codigo);
    toast.success(`Matéria-prima "${mpParaExcluir.nome}" marcada como ELIMINADA.`);
    setMpParaExcluir(null);
  };
          ...
          <div className="overflow-y-auto custom-scrollbar flex-grow">
            <table className="w-full text-left border-collapse table-fixed">
              <thead className="sticky top-0 z-10">
                <tr className="bg-gold/15 backdrop-blur-md border-b border-gold/20">
                  ...
                  <th className="py-2 px-3 font-serif font-semibold text-gold-dark tracking-wide text-[10px] w-20">Dimensões</th>
                  <th className="py-2 px-3 font-serif font-semibold text-gold-dark tracking-wide text-[10px] w-20">Custo Chapa</th>
                  <th className="py-2 px-3 font-serif font-semibold text-gold-dark tracking-wide text-[10px] w-28">Início Validade Custo</th>
                  <th className="py-2 px-3 font-serif font-semibold text-gold-dark tracking-wide text-[10px] w-24">Operador</th>
                  <th className="py-2 px-3 font-serif font-semibold text-gold-dark tracking-wide text-[10px] w-20">Custo/mm²</th>
              ...
              <tbody>
                {filteredAndSortedMP.map((mp) => {
                  const isEliminado = mp.status === 'ELIMINADO';
                  const ultimoHistorico = mp.historicoCustos?.[mp.historicoCustos.length - 1];
                  return (
                    <tr key={mp.codigo} className={`border-b border-gold/10 hover:bg-white/20 transition-all duration-300 group ${isEliminado ? 'bg-red-50/30' : ''}`}>
                      <td className={`py-2 px-3 text-[10px] font-medium ${isEliminado ? 'text-red-600' : 'text-gray-600'}`}>{mp.codigo}</td>
                      ...
                      <td className={`py-2 px-3 text-[10px] ${isEliminado ? 'text-red-600' : 'text-gray-600'}`}>{mp.largura}x{mp.altura}</td>
                      <td className={`py-2 px-3 text-[10px] ${isEliminado ? 'text-red-600' : 'text-gray-600'}`}>R$ {mp.custoChapa.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                      <td className={`py-2 px-3 text-[9px] ${isEliminado ? 'text-red-600' : 'text-gray-600'}`}>
                        {ultimoHistorico?.data || mp.dataCadastro}
                      </td>
                      ...
                      <td className={`py-2 px-3 text-[10px] font-mono font-bold ${isEliminado ? 'text-red-600' : 'text-gold-dark'}`}>
                        {mp.custoPorMm2.toFixed(6).replace('.', ',')}
                      </td>
            ...
            {filteredAndSortedMP.length === 0 && (
              <div className="flex-grow flex flex-col items-center justify-center p-8 text-gray-400 h-full min-h-[200px]">
                ...
                <p className="font-serif italic text-sm">Nenhuma matéria-prima encontrada com os filtros aplicados.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-gold/30 animate-in fade-in zoom-in duration-200">
            <div className="flex flex-col items-center text-center gap-4">
              ...
              <p className="text-gray-600">
                Deseja realmente eliminar o cadastro da matéria-prima <span className="font-bold text-gray-800">{mpParaExcluir.nome}</span>? 
                O registro permanecerá no histórico como ELIMINADO.
              </p>
              <div className="flex gap-3 w-full mt-6">
                <button 
                  onClick={() => setMpParaExcluir(null)}
