...
import { Pedido, Cliente, MateriaPrima } from '../SistemasContext';
import { format } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { safeFormat } from '../utils/dateUtils';

interface RelatorioPedidoA5Props {
  pedido: Pedido;
  cliente: Cliente | undefined;
  materiasPrimas: MateriaPrima[];
}

export const RelatorioPedidoA5 = forwardRef<HTMLDivElement, RelatorioPedidoA5Props>(
  ({ pedido, cliente, materiasPrimas }, ref) => {
    const dataAtual = new Date();
    const isIgreja = pedido.itens.some(item => item.isIgreja);
      ...
      <div ref={ref} style={{ padding: '2rem', margin: '0 auto', width: '148mm', minHeight: '210mm', fontFamily: 'sans-serif', backgroundColor: '#ffffff', color: '#000000', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '2px solid #1f2937', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
          ...
          <div style={{ textAlign: 'right', fontSize: '0.75rem', color: '#4b5563' }}>
            ...
            <p style={{ margin: 0 }}>Operador: {pedido.operadorCriacao}</p>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '0.875rem', fontWeight: 'bold', color: '#1f2937', marginBottom: '0.5rem', textTransform: 'uppercase', borderBottom: '1px solid #1f2937', paddingBottom: '0.25rem', marginTop: 0 }}>Itens do Pedido</h3>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '2rem' }}>
          <div style={{ width: '50%', backgroundColor: '#f9fafb', padding: '1rem', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <span style={{ fontSize: '0.875rem', fontWeight: 600, color: '#4b5563' }}>Sinal Pago:</span>
              <span style={{ fontSize: '0.875rem', fontWeight: 500, color: '#1f2937' }}>R$ {pedido.sinalPago.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
              <span style={{ fontSize: '0.875rem', fontWeight: 600, color: '#4b5563' }}>Restante:</span>
              <span style={{ fontSize: '0.875rem', fontWeight: 500, color: '#1f2937' }}>R$ {(pedido.totalGeral - pedido.sinalPago).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #d1d5db', paddingTop: '0.5rem' }}>
              ...
              <span style={{ fontSize: '1.125rem', fontWeight: 'bold', color: '#111827' }}>R$ {pedido.totalGeral.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom: '1.5rem' }}>
          <p style={{ fontSize: '0.875rem', color: '#1f2937', marginBottom: '0.5rem', marginTop: 0 }}>
            <span style={{ fontWeight: 'bold' }}>Data de Entrega Prevista:</span> {safeFormat(pedido.dataEntrega, 'dd/MM/yyyy', 'A combinar')}
          </p>
          {pedido.status === 'Entregue' && (
            <div style={{ backgroundColor: '#f0fdf4', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid #bbf7d0', marginBottom: '0.5rem' }}>
              <p style={{ fontSize: '0.875rem', color: '#166534', fontWeight: 'bold', margin: '0 0 0.25rem 0' }}>DETALHES DA ENTREGA</p>
              <p style={{ fontSize: '0.875rem', color: '#1f2937', margin: '0 0 0.25rem 0' }}>
                <span style={{ fontWeight: 'bold' }}>Data Efetiva:</span> {safeFormat(pedido.dataEntregaEfetiva, 'dd/MM/yyyy', '-')}
              </p>
              <p style={{ fontSize: '0.875rem', color: '#1f2937', margin: '0 0 0.25rem 0' }}>
                <span style={{ fontWeight: 'bold' }}>Recebido por:</span> {pedido.recebidoPor || '-'}
              </p>
              <p style={{ fontSize: '0.875rem', color: '#1f2937', margin: 0 }}>
                <span style={{ fontWeight: 'bold' }}>Saldo Pago:</span> R$ {pedido.pagamentoSaldo?.toLocaleString('pt-BR', { minimumFractionDigits: 2 })} ({pedido.formaPagamentoSaldo})
              </p>
            </div>
          )}
          {pedido.status === 'CANCELADO' && (
            <div style={{ backgroundColor: '#fef2f2', padding: '0.75rem', borderRadius: '0.5rem', border: '1px solid #fecaca', marginBottom: '0.5rem' }}>
              <p style={{ fontSize: '0.875rem', color: '#991b1b', fontWeight: 'bold', margin: '0 0 0.25rem 0' }}>PEDIDO CANCELADO</p>
              <p style={{ fontSize: '0.875rem', color: '#1f2937', margin: 0 }}>
                <span style={{ fontWeight: 'bold' }}>Justificativa:</span> {pedido.justificativaCancelamento || '-'}
              </p>
            </div>
          )}
          <p style={{ fontSize: '0.875rem', color: '#1f2937', marginBottom: '0.5rem', marginTop: 0 }}>
            <span style={{ fontWeight: 'bold' }}>Forma de Pagamento Inicial:</span> {pedido.formaPagamento}
          </p>
          <p style={{ fontSize: '0.875rem', color: '#1f2937', marginBottom: '1rem', marginTop: 0 }}>
            <span style={{ fontWeight: 'bold' }}>Status Atual:</span> <span style={{ textTransform: 'uppercase', fontWeight: 'bold', color: pedido.status === 'CANCELADO' ? '#ef4444' : '#1f2937' }}>{pedido.status}</span>
          </p>
        </div>
...
RelatorioPedidoA5.displayName = 'RelatorioPedidoA5';
